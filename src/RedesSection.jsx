...
const posts = [
  {tipo:'educativo',titulo:'3 errores al digitalizar una pyme que cuestan más de lo que ahorran',estado:'Listo para publicar',fecha:'Lun 21 · 09:00',texto:'Cuando una pyme digitaliza mal, el problema no es la tecnología. Es el orden. He visto 3 errores que se repiten siempre:\n\n1. Digitalizar procesos rotos en lugar de arreglarlos primero\n2. Comprar herramientas antes de saber qué necesitas\n3. No formar al equipo antes del cambio\n\n¿Te suena alguno? En Arrieta Consultores lo abordamos antes de tocar ninguna herramienta.\n\n#Digitalización #PymesVascas #Consultoría'},
  {tipo:'caso',titulo:'Cómo reduje el tiempo de reporting de una empresa de 18 personas de 3h a 20min',estado:'Borrador IA',fecha:'Mié 23 · 08:30',texto:'Borrador IA en proceso...'},
  {tipo:'personal',titulo:'Lo que aprendí en 5 años como consultor independiente',estado:'Idea aprobada',fecha:'Sáb 27 · 11:00',texto:'Idea pendiente de desarrollar...'},
]

function ModalPost({ open, onClose, post }) {
  if (!post) return null
  return (
    <Modal open={open} onClose={onClose} maxWidth={560} title="Revisar y publicar" subtitle={post.fecha}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <span style={{fontSize:'0.7rem',fontWeight:600,padding:'3px 8px',borderRadius:100,background:postBgs[post.tipo],color:postColors[post.tipo]}}>{post.tipo}</span>
        <span style={{fontSize:'0.72rem',color:'rgba(28,45,68,0.5)'}}>{post.estado}</span>
      </div>
      <div className="dm-field"><div className="dm-label">Texto del post</div><textarea className="dm-textarea" style={{minHeight:180}} defaultValue={post.texto}/></div>
      <div className="dm-row">
        <div className="dm-field"><div className="dm-label">Publicar en</div><select className="dm-select"><option>LinkedIn</option><option>LinkedIn + Instagram</option></select></div>
        <div className="dm-field"><div className="dm-label">Fecha y hora</div><input className="dm-input" type="datetime-local" defaultValue="2026-04-21T09:00"/></div>
      </div>
...
export default function RedesSection() {
  ...
  return (
    <div>
      <ModalPost open={!!postModal} onClose={() => setPostModal(null)} post={postModal} />
      ...
      {tab === 'plan' && (
        <div style={{background:'#1C2D44',borderRadius:14,padding:24}}>
          <div style={{display:'flex',alignItems:'flex-start',gap:16,marginBottom:20}}>
            ...
            <div>
              <div style={{fontSize:'0.9rem',fontWeight:600,color:'#FAF7F2',marginBottom:6}}>Plan de contenidos · Arrieta Consultores · Abril 2026</div>
              <div style={{fontSize:'0.78rem',color:'rgba(250,247,242,0.65)',lineHeight:1.5}}>He analizado tu brief, sector y audiencia en LinkedIn. Plan optimizado para pymes 10-50 empleados del sector servicios — donde tienes mayor engagement histórico.</div>
            </div>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:20}}>
            {[{lbl:'Frecuencia óptima',val:'3 posts/semana — tu audiencia satura con 4+ y pierde alcance con 2-'},{lbl:'Horarios con más engagement',val:'Martes y jueves · 8:00-9:30h — momento de revisión diaria de CEOs pymes'},{lbl:'Mix editorial',val:'40% educativo · 30% casos reales · 20% personal · 10% promo'}].map((r,i) => (
      ...
      {tab === 'calendario' && (
        <div className="dia-card">
          <div className="dia-card-head"><div className="dia-card-ttl">Calendario editorial · Abril 2026</div><div className="dia-card-sub">13 posts programados · Pulsa en un día para ver el post</div></div>
          <div className="rs-cal">
            {['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'].map(d => <div key={d} className="rs-cal-head">{d}</div>)}
            {diasMes.map((d,i) => (
              <div key={i} className={`rs-cal-day${d.today?' today':''}${!d.n?' empty':''}`} style={{cursor:d.post?'pointer':'default'}} onClick={() => d.post && setPostModal(posts[0])}>
                {d.n && <span className="rs-cal-num">{d.n}</span>}
                {d.post && <span style={{display:'block',fontSize:'0.6rem',fontWeight:600,padding:'2px 4px',borderRadius:4,background:postBgs[d.post],color:postColors[d.post],marginTop:2,overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis'}}>{d.postTxt}</span>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="dia-grid">
          ...
          <div className="dia-card">
            ...
            <div className="ins-list" style={{gap:9}}>
              ...
              <div className="ins-item"><div className="ins-ico warn"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div><div className="ins-body"><div className="ins-title">Publicar en fin de semana baja alcance 40%</div><div className="ins-desc">Mueve los posts del sábado al martes o jueves.</div></div></div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'posts' && (
        <div className="dia-grid">
          {posts.map((p,i) => (
            <div key={i} className="dia-card" style={{cursor:'pointer'}} onClick={() => setPostModal(p)}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
