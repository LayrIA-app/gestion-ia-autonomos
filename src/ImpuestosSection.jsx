...
const modelos1T = [
  {num:'303',desc:'IVA trimestral',origen:'Facturas emitidas − IVA soportado gastos',imp:'1.683 €',eBg:'rgba(198,93,74,0.1)',eColor:'#C65D4A',estado:'⚡ Pagar',bg:'rgba(198,93,74,0.03)'},
  {num:'130',desc:'IRPF fraccionado autónomos',origen:'20% de los rendimientos netos 1T',imp:'218 €',eBg:'rgba(198,93,74,0.1)',eColor:'#C65D4A',estado:'⚡ Pagar',bg:'rgba(198,93,74,0.03)'},
  {num:'111',desc:'Retenciones IRPF (facturas con retención)',origen:'Retenciones aplicadas en facturas',imp:'1.425 €',eBg:'rgba(198,93,74,0.1)',eColor:'#C65D4A',estado:'⚡ Pagar',bg:'rgba(198,93,74,0.03)'},
  {num:'390',desc:'Resumen anual IVA · solo Diciembre',origen:'Compila los 4 trimestres 303',imp:'—',eBg:'rgba(28,45,68,0.07)',eColor:'rgba(28,45,68,0.5)',estado:'Dic 2026',bg:''},
]
              ...
              <tbody>
                {tabT === 't1' && modelos1T.map((m,i) => (
                  <tr key={i} style={{borderBottom:'0.5px solid rgba(28,45,68,0.06)',background:m.bg}}>
                    ...
                    <td style={{padding:'11px 12px',textAlign:'right',fontFamily:'var(--serif)',fontSize:'1rem',fontWeight:500,color:m.eColor==='#C65D4A'?'#C65D4A':'#1C2D44'}}>{m.imp}</td>
                    ...
                    <td style={{padding:'11px 12px'}}>{m.eColor==='#C65D4A' && <button style={{padding:'4px 10px',background:'#C65D4A',border:'none',borderRadius:6,fontFamily:'var(--sans)',fontSize:'0.72rem',fontWeight:500,color:'#FAF7F2',cursor:'pointer'}}>Pagar →</button>}</td>
                  </tr>
                ))}
                {tabT !== 't1' && (
                  <tr><td colSpan={6} style={{padding:'16px 12px',fontSize:'0.84rem',color:'rgba(28,45,68,0.6)'}}>La IA calculará los importes automáticamente cuando llegue el trimestre.</td></tr>
                )}
