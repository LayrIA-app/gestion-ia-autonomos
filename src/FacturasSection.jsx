...
const thS = {padding:'10px 12px',textAlign:'left',fontSize:'0.68rem',fontWeight:600,letterSpacing:'0.08em',textTransform:'uppercase',color:'rgba(28,45,68,0.45)'}
const thR = {...thS, textAlign:'right'}

const facturas = [
  {num:'F-2026-043',cliente:'Metalúrgica Goi',concepto:'Retainer mayo 2026',fecha:'18 abr 2026',base:'2.400 €',iva:'504 €',total:'2.544 €',estado:'✦ IA lista',estadoBg:'rgba(46,90,140,0.12)',estadoColor:'#2E5A8C',bg:'rgba(46,90,140,0.04)',acciones:[{l:'Editar'},{l:'Revisar y enviar →',dark:true}]},
  {num:'F-2026-041',cliente:'Digiform SL',concepto:'Consultoría abril 2026',fecha:'17 abr 2026',base:'2.400 €',iva:'504 €',total:'2.544 €',estado:'⚡ Vencida',estadoBg:'rgba(198,93,74,0.1)',estadoColor:'#C65D4A',bg:'rgba(198,93,74,0.02)',acciones:[{l:'Editar'},{l:'Recordar'}]},
  {num:'F-2026-038',cliente:'Digiform SL',concepto:'Consultoría marzo 2026',fecha:'09 abr 2026',base:'1.800 €',iva:'378 €',total:'1.908 €',estado:'⚡ Vencida',estadoBg:'rgba(198,93,74,0.1)',estadoColor:'#C65D4A',bg:'rgba(198,93,74,0.02)',acciones:[{l:'Editar'},{l:'Recordar'}]},
  {num:'F-2026-035',cliente:'Metalúrgica Goi',concepto:'Retainer abril 2026',fecha:'01 abr 2026',base:'2.400 €',iva:'504 €',total:'2.544 €',estado:'Cobrada ✓',estadoBg:'rgba(34,160,107,0.12)',estadoColor:'#22A06B',bg:'',acciones:[{l:'PDF'},{l:'Reenviar'}]},
  {num:'F-2026-031',cliente:'Metalúrgica Goi',concepto:'Consultoría marzo 2026',fecha:'15 mar 2026',base:'2.400 €',iva:'504 €',total:'2.544 €',estado:'Cobrada ✓',estadoBg:'rgba(34,160,107,0.12)',estadoColor:'#22A06B',bg:'',acciones:[{l:'PDF'}]},
  {num:'F-2026-028',cliente:'Metalúrgica Goi',concepto:'Consultoría febrero 2026',fecha:'01 mar 2026',base:'2.400 €',iva:'504 €',total:'2.544 €',estado:'Cobrada ✓',estadoBg:'rgba(34,160,107,0.12)',estadoColor:'#22A06B',bg:'',acciones:[{l:'PDF'}]},
  {num:'F-2026-018',cliente:'Metalúrgica Goi',concepto:'Consultoría enero 2026',fecha:'01 feb 2026',base:'2.400 €',iva:'504 €',total:'2.544 €',estado:'Cobrada ✓',estadoBg:'rgba(34,160,107,0.12)',estadoColor:'#22A06B',bg:'',acciones:[{l:'PDF'}]},
]
      ...
      <div className="dia-grid">
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.84rem',minWidth:700}}>
              <thead><tr style={{borderBottom:'1.5px solid rgba(28,45,68,0.08)'}}>
                <th style={thS}>Nº Factura</th><th style={thS}>Cliente</th><th style={thS}>Concepto</th><th style={thS}>Fecha</th>
                <th style={thR}>Base</th><th style={thR}>IVA</th><th style={thR}>Total</th><th style={thS}>Estado</th><th style={{padding:'10px 12px'}}></th>
              </tr></thead>
              <tbody>
                {facturas.map((f,i) => (
                  <tr key={i} style={{borderBottom:'0.5px solid rgba(28,45,68,0.06)',background:f.bg}}>
                    ...
                    <td style={{padding:12}}><span style={{fontSize:'0.72rem',fontWeight:600,padding:'3px 10px',borderRadius:100,background:f.estadoBg,color:f.estadoColor}}>{f.estado}</span></td>
                    <td style={{padding:12}}>
                      <div style={{display:'flex',gap:6}}>
                        {f.acciones.map((a,j) => a.dark
                          ? <button key={j} style={{padding:'4px 10px',background:'#1C2D44',border:'none',borderRadius:6,fontFamily:'var(--sans)',fontSize:'0.72rem',fontWeight:500,color:'#FAF7F2',cursor:'pointer'}}>{a.l}</button>
                          : <button key={j} className="btn-ghost" style={{padding:'4px 10px',fontSize:'0.72rem'}}>{a.l}</button>
                        )}
                      </div>
                    </td>
                  </tr>
        ...
        <div className="dia-card">
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            {[
              {label:'IVA repercutido (21%)',sub:'Sobre base de 9.500 €',val:'1.995 €',bg:'rgba(188,212,232,0.15)',dark:false},
              {label:'IVA soportado (gastos)',sub:'Facturas de proveedores',val:'312 €',bg:'rgba(212,165,116,0.1)',dark:false},
              {label:'A ingresar modelo 303',sub:'Vence 20 de abril',val:'1.683 €',bg:'#1C2D44',dark:true},
              {label:'IRPF retenido (15%)',sub:'Modelo 130',val:'218 €',bg:'rgba(28,45,68,0.04)',dark:false},
            ].map((r,i) => (
              <div key={i} style={{display:'flex',justifyContent:'space-between',padding:10,background:r.bg,borderRadius:8}}>
                <div><div style={{fontSize:'0.78rem',fontWeight:500,color:r.dark?'#FAF7F2':'#1C2D44'}}>{r.label}</div><div style={{fontSize:'0.72rem',color:r.dark?'rgba(250,247,242,0.55)':'rgba(28,45,68,0.5)'}}>{r.sub}</div></div>
                <div style={{fontFamily:'var(--serif)',fontSize:'1.1rem',fontWeight:500,color:r.dark?'#FAF7F2':'#1C2D44'}}>{r.val}</div>
              </div>
            ))}
          </div>
        </div>
